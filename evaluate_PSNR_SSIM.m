...
for idx_set = 1:num_set
    ...
    % Rain100L
    file_path = strcat(['dataset/Rain12/result/']);
    % file_path = strcat(['test/']);
    gt_path = strcat('dataset/Rain12/target/');
    if ~isfolder(file_path)
        error('The input folder does not exist: %s', file_path);
    end
    
    if ~isfolder(gt_path)
        error('The target folder does not exist: %s', gt_path);
    end 
...
fprintf('The result dictionary is : %s \n', file_path);
