...
class RainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sdr_img_path = os.path.join(self.image_dir, 'sdr_fuse', self.label_list[idx])
        sdr_img_path = sdr_img_path.replace(".png", "")
        sdr_img_path = sdr_img_path.replace(".jpg", "")
        # sdr_img_path = os.path.join(sdr_img_path,".png")
        sdr_img_path = sdr_img_path + ".png"
        sdr_img = Image.open(sdr_img_path).convert("RGB")
        ...
        rain_mask = F.to_tensor(ldgp_img)

        ori_input_img = input_img.clone()
        ori_sdr_img = sdr_img.clone()
        ori_rain_mask = rain_mask.clone()

        _, h, w = input_img.shape
        sdr_img, input_img, rain_mask, _, _ = self.random_crop_pair(ori_sdr_img, ori_input_img, ori_rain_mask, ori_rain_mask, ori_sdr_img, crop_size=self.crop_size)
        _, another_input_img_1, another_mask_1, _, _ = self.random_crop_pair(ori_sdr_img, ori_input_img, ori_rain_mask, ori_rain_mask, ori_sdr_img, crop_size=self.crop_size)
        _, another_input_img_2, another_mask_2, _, _ = self.random_crop_pair(ori_sdr_img, ori_input_img, ori_rain_mask, ori_rain_mask, ori_sdr_img, crop_size=self.crop_size)
...
def Addrain_dataloader(path, batch_size=1, num_workers=0):
    dataloader = DataLoader(
        Addrain_Dataset(path),
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        pin_memory=True,
...
class Addrain_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if h > self.crop_size and w > self.crop_size:
            # sdr_img, input_img, rain_mask = self.random_crop_pair(sdr_img, input_img, rain_mask, crop_size=self.crop_size)
            sdr_img, input_img, rain_mask, _, _ = self.random_crop_pair(self.sdr_img, self.input_img, self.rain_mask, self.non_rain_mask, self.new_sdr_img, crop_size=self.crop_size)
            _, another_input_img, another_rain_mask, _, _ = self.random_crop_pair(self.sdr_img, self.input_img, self.rain_mask, self.non_rain_mask, self.new_sdr_img, crop_size=self.crop_size)
            # _, _, non_rain_mask_2, _, _ = self.random_crop_pair(self.sdr_img, self.input_img, self.rain_mask, self.non_rain_mask, self.new_sdr_img, crop_size=self.crop_size)
            sdr_img, input_img, rain_mask, another_rain_mask, another_input_img = self.random_flip(sdr_img, input_img, rain_mask, another_rain_mask, another_input_img)
        else:
            factor = 16
            H,W = ((h+factor)//factor)*factor, ((w+factor)//factor)*factor
            padh = H-h if h%factor!=0 else 0
        ...
        return sdr_img, input_img, rain_mask, another_rain_mask, another_input_img
