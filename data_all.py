...
class RainAllDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = Image.open(os.path.join(self.image_dir, 'input', self.image_list[idx])).convert("RGB")
        label = Image.open(os.path.join(self.image_dir, 'sdr_fuse', self.label_list[idx])).convert("RGB")
        ldgp = Image.open(os.path.join(self.image_dir, 'sdr_fuse', self.label_list[idx])).convert("RGB")
        random_sdr_index = random.randint(0, 49)
        sdr = Image.open(os.path.join(self.image_dir, 'sdr_fuse', self.label_list[idx])).convert("RGB")

        # sdr = Image.open(os.path.join(self.image_dir, 'sdr_fuse', self.label_list[idx][:-4], str(random_sdr_index)+".png")).convert("RGB")
